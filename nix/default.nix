{
  ...
  runtimeDeps =
    ...
    ++ extraRuntimeDeps
    ++ lib.optional withCli ferret-cli;
  ...
  extras = stdenv.mkDerivation {
    ...
    name = "ferret-extras${lib.optionalString debug "-debug"}";
  ...
  plugin = stdenv.mkDerivation {
    inherit cmakeBuildType;
    name = "ferret-qml-plugin${lib.optionalString debug "-debug"}";
    src = lib.fileset.toSource {
      root = ./..;
...
in
  stdenv.mkDerivation {
    ...
    pname = "ferret-shell${lib.optionalString debug "-debug"}";
    src = ./..;
    ...
    cmakeFlags =
      [
        ...
        (lib.cmakeFeature "INSTALL_QSCONFDIR" "${placeholder "out"}/share/ferret-shell")
      ]
      ++ cmakeVersionFlags;
    ...
    postInstall = ''
      makeWrapper ${quickshell}/bin/qs $out/bin/ferret-shell \
      	--prefix PATH : "${lib.makeBinPath runtimeDeps}" \
      	--set FONTCONFIG_FILE "${fontconfig}" \
      	--set FERRET_LIB_DIR ${extras}/lib \
        --set FERRET_XKB_RULES_PATH ${xkeyboard-config}/share/xkeyboard-config-2/rules/base.lst \
      	--add-flags "-p $out/share/ferret-shell"

      mkdir -p $out/lib
      ln -s ${extras}/lib/* $out/lib/

      # Ensure wrap_term_launch.sh is executable
      chmod 755 $out/share/ferret-shell/assets/wrap_term_launch.sh
    '';
    ...
    meta = {
      ...
      homepage = "https://github.com/ferret-dots/shell";
      license = lib.licenses.gpl3Only;
      mainProgram = "ferret-shell";
    };
  }
